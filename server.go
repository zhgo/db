...
func (e *Server) prepare(sql string) (*sql.Stmt, error) {
    ...
    stmt, err := dbObjects[e.DSN].Prepare(sql)
    if err != nil {
        return nil, err
    }
...
func (e *Server) connect() error {
    db, s := dbObjects[e.DSN]
    // err := db.Ping(); err != nil
    if s == false || db == nil {
        var err error
        dbObjects[e.DSN], err = sql.Open(e.Type, e.DSN)
        if err != nil {
            return err
        }
    }
